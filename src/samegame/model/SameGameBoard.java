...
import static samegame.common.Constants.BOARD_DIMENSION;
...
public class SameGameBoard
{
   ...
   */
   ...
   public int deleteBlocks(int row, int col)
   {
      ...
      // with the same color
      int count = -1;
      if ((row - 1 >= 0 && arrBoard[row - 1][col] == color)
            || (row + 1 < rows && arrBoard[row + 1][col] == color)
            ...
            || (col + 1 < columns && arrBoard[row][col + 1] == color))
      {
         ...
         count += deleteNeighborBlocks(row - 1, col, color,
         ...
         count += deleteNeighborBlocks(row, col - 1, color,
   ...
   private int deleteNeighborBlocks(int row, int col, int color,
         DIRECTION direction)
   {
   ...
   private void compactBoard()
   {
      ...
      while (nextEmptyCol < columns && nextOccupiedCol < columns)
      {
         ...
         while (nextEmptyCol < columns
               && arrBoard[rows - 1][nextEmptyCol] != 0)
            nextEmptyCol++;
         ...
         if (nextEmptyCol < columns)
         {
            ...
            while (nextOccupiedCol < columns
                  && arrBoard[rows - 1][nextOccupiedCol] == 0)
               nextOccupiedCol++;
